...
if (isset($_SESSION['user_id'])) {
    ...
    // Fetch grade
    $sql = "SELECT module1_lab_grade FROM grades WHERE user_id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $user_id);
    $stmt->execute();
    $stmt->store_result();
    ...
    $sql = "UPDATE progress SET module1_completed = 1 WHERE user_id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $user_id);
    $stmt->execute();
...
// Load XML file
$xml = simplexml_load_file("xmlfile/experiment1.xml") or die("Error: Cannot load experiment1.xml");
...
if (isset($_GET['edit']) && $_GET['edit'] === 'true') {
    echo "<script>window.addEventListener('load', openEditPage);</script>";
}
?>
  ...
  <?php endif; ?>
    ...
    <script>
        function proceedToLab() {
            window.location.href = "experiment1_lab.php";
        }
        ...
        function openEditPage() {
            ...
            const xmlFilePath = 'xmlfile/experiment1.xml'; // The path to the current XML file
