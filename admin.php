...
$sql_accounts = "SELECT user_id, username, email FROM users";
...
<body>
    ...
    <div class="content">
        <div class="admin-buttons">
            ...
            <button onclick="window.location.href='editaccounts.php'">Edit Accounts</button>
        </div>

        <h1>Accounts</h1>
        <a href="add_account.php" onclick="openSmallWindow('add_account.php', 'Add Account', 500, 400); return false;" class="add-account-btn">Add New Account</a>
        ...
        <table>
            ...
            <tbody>
                <?php
                // Fetch and display accounts from the database
                $conn = new mysqli($servername, $db_username, $db_password, $db_name);
                if ($conn->connect_error) {
                    die("Connection failed: " . $conn->connect_error);
                }
                ...
                if ($result_accounts->num_rows > 0) {
                    while ($row = $result_accounts->fetch_assoc()) {
                        echo "<tr>
                                ...
                                <td>{$row['email']}</td>
                                <td>
                                    <button class='action-btn remove' onclick='showRemovePopup({$row['user_id']})'>Remove</button>
                                </td>
                            </tr>";
                    }
                } else {
