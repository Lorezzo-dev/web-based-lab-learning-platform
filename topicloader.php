...
if (isset($_GET['file'])) {
    ...
    $xml = simplexml_load_file("xmlfile/experiment1.xml") or die("Error: Cannot load the default XML file.");
}

// Extract content from the XML file
$title = $xml->title ?? "No Title";
$description = $xml->description ?? "No Description";
$subtopics = $xml->subtopics ?? [];
$video_src = $xml->video->source ?? "";
$video_description = $xml->video->description ?? "No Video Description";
$quiz = $xml->quiz ?? [];
  ...
  <?php endif; ?>
    ...
    <script>
        ...
        function openEditPage() {
            ...
            const xmlFilePath = <?php echo json_encode($filePath); ?>;
