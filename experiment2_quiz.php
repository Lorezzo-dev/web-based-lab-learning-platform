...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['grade'])) {
    ...
    // Prepare SQL statement to update module2_quiz_grade
    $sql = "UPDATE grades SET module2_quiz_grade = ? WHERE user_id = ?";
    ...
    $sql = "UPDATE progress SET module2quiz_completed = 1 WHERE user_id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $user_id); // Bind parameters

    // Execute the update query
    if ($stmt->execute()) {
        echo "Module 2 quiz completion status updated successfully";
    } else {
...
$xml = simplexml_load_file('xmlfile/quiz2data.xml') or die("Error: Cannot load quiz data");
  ...
  <div id="editPopup" class="edit-popup">
      <div class="popup-content">
        ...
        <div id="quiz-container">
            ...
            <div id="results"><button id="proceed-button" class="lab-btn" onclick="proceedToLab()" style="display: none;">Proceed to Module 3</button></div>
        </div>
    ...
    function proceedToLab() {
        window.location.href = "experiment3.php"; // Replace with your module 2 page
    }

    function openEditPage() {
        const editPopup = document.getElementById('editPopup');
        const editIframe = document.getElementById('editIframe');
        const xmlFilePath = 'xmlfile/quiz2data.xml'; // The path to the current XML file
