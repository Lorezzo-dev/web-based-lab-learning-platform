...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['grade'])) {
    ...
    $sql = "UPDATE grades SET module3_quiz_grade = ? WHERE user_id = ?";
    ...
    $sql = "UPDATE progress SET module3quiz_completed = 1 WHERE user_id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $user_id); // Bind parameters

    // Execute the update query
    if ($stmt->execute()) {
        echo "Module 3 quiz completion status updated successfully";
    } else {
...
$xml = simplexml_load_file('xmlfile/quiz3data.xml') or die("Error: Cannot load quiz data");
  ...
  <div id="editPopup" class="edit-popup">
      <div class="popup-content">
        ...
        <div id="quiz-container">
            ...
            <div id="results"><button id="proceed-button" class="lab-btn" onclick="proceedToLab()" style="display: none;">Proceed to Module 4</button></div>
        </div>


    <div class="custom-prompt" id="customPrompt">
        <div class="prompt-content">
            <h2>Proceed to Module 4</h2>
    ...
    function proceedToLab() {
        window.location.href = "experiment4.php"; // Replace with your module 2 page
    }

    function openEditPage() {
        const editPopup = document.getElementById('editPopup');
        const editIframe = document.getElementById('editIframe');
        const xmlFilePath = 'xmlfile/quiz3data.xml'; // The path to the current XML file
