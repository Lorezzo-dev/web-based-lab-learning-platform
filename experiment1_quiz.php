...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['grade'])) {
    ...
    // Prepare SQL statement to update module1_quiz_grade
    $sql = "UPDATE grades SET module1_quiz_grade = ? WHERE user_id = ?";
    ...
    // Prepare SQL statement to update module1quiz_completed to 1
    $sql = "UPDATE progress SET module1quiz_completed = 1 WHERE user_id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $user_id); // Bind parameters

    // Execute the update query
    if ($stmt->execute()) {
        echo "Module 1 quiz completion status updated successfully";
    } else {
...
$xml = simplexml_load_file('xmlfile/quiz1data.xml') or die("Error: Cannot load quiz data");
...
<head>
    ...
    .point-container {
        ...
        left: 50%; /* Center above the question */
  ...
  <div id="editPopup" class="edit-popup">
      <div class="popup-content">
        ...
        <div id="quiz-container">
            ...
            <div id="results"><button id="proceed-button" class="lab-btn" onclick="proceedToLab()" style="display: none;">Proceed to Module 2</button></div>
        </div>


    <div class="custom-prompt" id="customPrompt">
        <div class="prompt-content">
            <h2>Proceed to Module 2</h2>
            <p>Do you want to proceed to the next module?</p>
    ...
    function proceedToLab() {
        window.location.href = "experiment2.php"; // Replace with your module 2 page
    }

    function openEditPage() {
        const editPopup = document.getElementById('editPopup');
        const editIframe = document.getElementById('editIframe');
        const xmlFilePath = 'xmlfile/quiz1data.xml'; // The path to the current XML file
